...
const Header = () => {
  ...
  return (
    <Navbar bg="dark" className='navbar-dark' expand="lg">
      <Container>
        <Navbar.Brand href="/">1001 Best</Navbar.Brand>
                    ...
                    <ul className="list-group">
                      {
                        !searchvalues.length ? "":product.filter(value=>value.title.toLocaleLowerCase().includes(searchvalues)).map(item => (
                         ...
                         <a href={`/products/${item.id}`}> <li className="list-group-item d-flex justify-content-between" key={item.id}>{item.title}<img src={item.images[0]} width={50} alt="" /></li></a>
                        )) 
                      }
